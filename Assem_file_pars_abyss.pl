...
system "ls assemb_abyss_T6_\*.txt > list_assem_out";# captures all the different outputs from the assemblethon program
...
foreach $out_file (@out_assem){# values in the array are the files
	...
	$out_file =~ m/assemb_abyss_T6_(\d+).txt/;# in order to define the keys in the hash before I assign a vaule to the hash table I need to make 	
